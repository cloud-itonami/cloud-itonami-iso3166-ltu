  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Vilnius CivicTech UAB"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Vilnius CivicTech UAB" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
  ...
  (testing "assess is never in any phase's :auto set -- always human approval, even when clean"
    (let [[db actor] (fresh)
      ...
      (let [r2 (approve! actor "t2")
        ...
        (is (some? assessment))
        (testing "the committed spec-basis cites the CURRENT CVP IS domain, never the legacy one"
          (is (re-find #"viesiejipirkimai\.lt" (:spec-basis assessment)))
          (is (not (facts/stale-cvp-is-domain? (:spec-basis assessment)))))))))
...
(deftest stale-cvp-is-domain-is-held-and-unoverridable
  (testing "citing the LEGACY pre-2024-12-01 CVP IS domain as live provenance -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
          res (exec-op actor "t4"
                    {:op :jurisdiction/assess :subject "eng-5" :stale-cvp-domain? true} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:stale-cvp-is-domain} (-> (store/ledger db) first :basis)))
      (is (nil? (store/assessment-of db "eng-5")) "no assessment written for the stale-domain proposal"))))

(deftest draft-without-assessment-is-held
  (testing "filing/draft before any jurisdiction assessment -> HOLD (evidence incomplete)"
    (let [[db actor] (fresh)
          res (exec-op actor "t5" {:op :filing/draft :subject "eng-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:evidence-incomplete} (-> (store/ledger db) first :basis))))))

(deftest vat-registration-missing-is-held-and-unoverridable
  (testing "missing VMI VAT registration when required -> HARD hold (flagship check, EUR 45,000 threshold)"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:vat-registration-missing} (-> (store/ledger db) last :basis)))
...
(deftest engagement-fee-mismatch-is-held
  (testing "claimed fee that doesn't equal base + months x rate -> HOLD"
    (let [[db actor] (fresh)
          _ (assess! actor "t7pre" "eng-3")
          _ (draft! actor "t7pre" "eng-3")
          res (exec-op actor "t7" {:op :filing/submit :subject "eng-3"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
...
(deftest filing-draft-and-submit-never-auto-commit-at-any-phase
  (testing "sweeping every phase, a clean filing/draft and filing/submit always interrupt -- never silently auto-commit"
    (doseq [ph [0 1 2 3]]
      (let [[_db actor] (fresh)
            ctx (assoc operator :phase ph)]
        (when (>= ph 2)
          (exec-op actor (str "phase" ph "-assess") {:op :jurisdiction/assess :subject "eng-1"} ctx)
          (approve! actor (str "phase" ph "-assess")))
        (let [r (exec-op actor (str "phase" ph "-draft") {:op :filing/draft :subject "eng-1"} ctx)]
          (is (not= :commit (get-in r [:state :disposition]))
              (str "phase " ph ": filing/draft must never silently commit")))))))
