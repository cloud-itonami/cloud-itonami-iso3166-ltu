(ns marketentry.marketentryllm
  ...
  the LTU public-sector market-entry compliance actor.
  ...
  `:no-spec?` and `:stale-cvp-domain?` are TEST-ONLY request flags that
  deliberately inject the two failure modes the governor MUST defend
  against: proposing a checklist for a jurisdiction with no official
  spec-basis, and citing the LEGACY (archive-only since 2024-12-01)
  CVP IS domain as if it were the live procurement platform.
...
(def ^:private legacy-cvp-is-url
  "The archive-only, pre-2024-12-01 CVP IS domain -- used ONLY to
  simulate the `:stale-cvp-domain?` failure-mode injection below.
  Never used as a real provenance value."
...
(defn- assess-jurisdiction
  ...
  with NO official spec-basis; `:stale-cvp-domain?` injects the
  failure mode of citing the LEGACY (archive-only) CVP IS domain
  instead of the current viesiejipirkimai.lt."
  [db {:keys [subject no-spec? stale-cvp-domain?]}]
  (let [e (store/engagement db subject)
    ...
    (cond
      ...
      {:summary    (str iso3 " (" (:owner-authority sb) ") 向け必要書類 "
                        (count (:required-evidence sb)) " 件を提案 (旧CVP ISドメイン引用 -- STALE)")
       :rationale  (str "旧ドメイン " legacy-cvp-is-url " を根拠として引用 -- 2024-12-01移行後はviesiejipirkimai.ltが正 (governorが拒否すべき提案)")
       :cites      [(:legal-basis sb) legacy-cvp-is-url]
       :effect     :assessment/set
       :value      {:jurisdiction iso3
                    :checklist (:required-evidence sb)
                    :spec-basis legacy-cvp-is-url
...
(defn- propose-submit
  ...
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けCVP ISポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "vat-registered?=" (:vat-registered? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-vat-registration? e))
                              (:vat-registered? e)))
                   0.9 0.3)}))
