(ns marketentry.store
  "SSoT for the LTU market-entry compliance actor, behind a `Store`
  ...
  Uses `kotoba-lang/langchain-store` for the shared EDN-blob codec /
  identity schema / event-log read-append helpers instead of hand-
  rolling `enc`/`dec*` (ADR-2607141600)."
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Vilnius CivicTech UAB" :portal "CVP IS"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-vat-registration? true :vat-registered? true
             :drafted? false :submitted? false
             :jurisdiction "LTU" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis Ventures" :portal "CVP IS"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-vat-registration? true :vat-registered? true
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Kaunas Public Works MB" :portal "CVP IS"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 999000.0
             :requires-vat-registration? true :vat-registered? true
             :drafted? false :submitted? false
             :jurisdiction "LTU" :status :intake}
    "eng-4" {:id "eng-4" :operator "Klaipeda Logistics MB" :portal "CVP IS"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-vat-registration? true :vat-registered? false
             :drafted? false :submitted? false
             :jurisdiction "LTU" :status :intake}
    "eng-5" {:id "eng-5" :operator "Siauliai Digital Services UAB" :portal "CVP IS"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 12
             :claimed-fee 700000.0
             :requires-vat-registration? true :vat-registered? true
             :drafted? false :submitted? false
             :jurisdiction "LTU" :status :intake}
    "eng-6" {:id "eng-6" :operator "Panevezys Health Systems UAB" :portal "CVP IS"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-vat-registration? vat-registered?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-vat-registration?)    (assoc :engagement/requires-vat-registration? requires-vat-registration?)
    (some? vat-registered?)               (assoc :engagement/vat-registered? vat-registered?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-vat-registration? :engagement/vat-registered?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-vat-registration? (boolean (:engagement/requires-vat-registration? m))
     :vat-registered? (boolean (:engagement/vat-registered? m))
