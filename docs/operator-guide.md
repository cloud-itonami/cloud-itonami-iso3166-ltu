...
2. Register the client's intake: business type, target public function,
   prior filing history in Lithuania if any.
3. Run the advisor in read-only mode against CVP IS
   (viesiejipirkimai.lt).
4. Compare the checklist against the client's current documentation
   (Register of Legal Entities status).
...
- named referral relationship with Lithuanian-licensed counsel or a
...
a working referral relationship with Lithuanian-licensed counsel or a
registered agent for whatever licensed representation the law of
Lithuania requires for actual public-procurement filings.
