# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Lithuania

## Classification

- Repository: `cloud-itonami-iso3166-ltu`
- ISO 3166: `LTU` (Lithuania)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Lithuanian
  public contract
...
- registration walkthrough for CVP IS (Central Public Procurement
  Information System, viesiejipirkimai.lt), the single mandatory portal
  for all Lithuanian public-procurement procedures above and below EU
  thresholds, run by the Public Procurement Office; free registration
...
- local-content / preferential-procurement navigation: EU-wide open
  ...
  member state), but Lithuanian-language submission is typically
  required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Lithuanian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Lithuania).
